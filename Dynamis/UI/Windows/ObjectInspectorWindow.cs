...
public sealed class ObjectInspectorWindow : IndexedWindow
{
    ...
    {
        using var id = ImRaii.PushId("###ClassInfo");
    ...
    private void DrawClassChangeButton()
    {
        ...
        if (hasInitialClass) {
            if (_vmInitialActualClass is not null) {
                ImGui.RadioButton("Original class:###initialClass", ref _vmReanalyze, 1);
        ...
        ImGui.RadioButton("Specified class:###newClass", ref _vmReanalyze, 2);
        using (ImRaii.PushIndent(2)) {
            confirm |= ImGui.InputText(
                "###newTypeName", ref _vmNewTypeName, 2048, ImGuiInputTextFlags.EnterReturnsTrue
            );
